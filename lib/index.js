...
module.exports = function(options) {
	...
	return function(req, res, next) {
		...
		if (req.headers && req.headers.signature) {
	    	...
	    	var signature = cryptojs.createHmac("md5", options.secret).update(data).digest("hex");
	    	
	    	if( req.headers.signature !== signature ) {
	    		return next(new SignatureError('signature_invalid', { message: 'This signature verification for this message has failed' }));
	    	}
